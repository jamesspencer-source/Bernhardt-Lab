...
7. Apply schema:
```bash
wrangler d1 execute bernhardt_lab_leaderboard --file=schema.sql
```
8. Deploy:
```bash
wrangler deploy
...
- `github-flat/envelope-escape-config.js`
2. Set:
```js
window.ENVELOPE_LEADERBOARD_URL = "https://<your-worker-subdomain>/leaderboard";
```
