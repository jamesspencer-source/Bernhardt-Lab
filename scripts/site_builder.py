...
SOURCE_LINK_LABELS = {
    "profile": ("Profile", "View external profile"),
    "directory": ("Institutional directory", "View institutional directory"),
    "third-party": ("Third-party directory", "View third-party directory"),
    "lab": ("Institutional lab page", "View institutional lab page"),
    "publication": ("Publication evidence", "View supporting publication"),
    "news": ("Organization announcement", "View organization announcement"),
    "evidence": ("Supporting source", "View supporting source"),
}
VERIFICATION_LINK_TYPES = set(SOURCE_LINK_LABELS)
...
def is_pubmed_evidence_url(value: object = "") -> bool:
...
def verification_link_type(verification: dict[str, Any]) -> str:
    host = (urlparse(clean_text(verification.get("url"))).hostname or "").lower()
    if host == "theorg.com" or host.endswith(".theorg.com"):
        return "third-party"
    if is_pubmed_evidence_url(verification.get("url")):
        return "publication"
    explicit_type = clean_text(verification.get("linkType")).lower()
    if explicit_type:
        return explicit_type
    return "profile"


def verification_source_prefix(link_type: str) -> str:
    return SOURCE_LINK_LABELS.get(link_type, SOURCE_LINK_LABELS["evidence"])[0]


def verification_link_label(link_type: str) -> str:
    return SOURCE_LINK_LABELS.get(link_type, SOURCE_LINK_LABELS["evidence"])[1]
...
def asset_version(filename: str) -> str:
    return hashlib.sha256((ASSETS_DIR / filename).read_bytes()).hexdigest()[:16]


def asset_versions() -> dict[str, str]:
    return {
        **{filename: asset_version(filename) for filename in HASHED_ASSETS},
...
def normalize_asset_versions(text: str) -> str:
    updated = text
    for filename, version in asset_versions().items():
        updated = re.sub(rf'(assets/{re.escape(filename)})(?:\?v=[^"#]+)?', rf"\1?v={version}", updated)
    return updated


def module_imports(root_prefix: str) -> dict[str, str]:
    prefix = root_prefix or "./"
    return {
        f"{prefix}assets/js/{path.name}": f"{prefix}assets/js/{path.name}?v={asset_version('js/' + path.name)}"
        for path in sorted((ASSETS_DIR / "js").glob("*.js"))
    }


def sync_asset_references() -> None:
    # Resolve every shared module independently; changing an entrypoint alone
    # does not invalidate its previously cached imports.
    pattern = re.compile(r'\n\s*<script type="importmap" id="site-module-map">.*?</script>', re.S)
    for path in iter_public_html_paths():
        text = pattern.sub("", read_text(path))
        text = normalize_asset_versions(text)
        if '<script type="module"' in text:
            base = FLAT_DIR if path.is_relative_to(FLAT_DIR) else ROOT
            prefix = "../" * (len(path.relative_to(base).parts) - 1)
            payload = json.dumps({"imports": module_imports(prefix)}, separators=(",", ":"))
            text = text.replace("</head>", f'<script type="importmap" id="site-module-map">{payload}</script>\n  </head>')
...
def validate_people_plain_text(person: dict[str, Any]) -> None:
    ...
    link_type = clean_text(verification.get("linkType")).lower()
    if link_type and link_type not in VERIFICATION_LINK_TYPES:
        allowed = ", ".join(sorted(VERIFICATION_LINK_TYPES))
        raise RuntimeError(f"Person {slug} verification.linkType must be one of: {allowed}")
...
def resolve_featured_alumni(people: list[dict[str, Any]]) -> list[dict[str, Any]]:
    by_slug = {person["slug"]: person for person in people}
    resolved = []
    seen = set()
    for item in load_featured_alumni_items():
        slug = clean_text(item.get("profileSlug"))
        person = by_slug.get(slug)
        if not person or person["status"] != "alumni":
            raise RuntimeError(f"Featured alumnus must reference an alumni record: {slug}")
        if slug in seen:
            raise RuntimeError(f"Duplicate featured alumnus: {slug}")
        seen.add(slug)
        allowed = {"profileSlug", "roleInLab", "currentRoleOverride", "sourceLabel", "showSource"}
        if set(item) - allowed:
            raise RuntimeError(f"Featured entry contains duplicated or unknown fields: {slug}")
        for key in ("roleInLab", "currentRoleOverride", "sourceLabel"):
            if key in item and (
                not isinstance(item[key], str) or not item[key].strip() or HTML_TAG_PATTERN.search(html.unescape(item[key]))
            ):
                raise RuntimeError(f"Featured {key} must be non-empty plain text: {slug}")
        if "showSource" in item and not isinstance(item["showSource"], bool):
            raise RuntimeError(f"Featured showSource must be boolean: {slug}")
        verification = person.get("verification") or {}
        # Retain explicitly approved short labels, but never duplicate identities or URLs.
        resolved.append({
            "profileSlug": slug,
            "name": person["name"],
            "roleInLab": item.get("roleInLab", person["labRole"]),
            "currentRole": item.get("currentRoleOverride", person.get("currentRole", "")),
            "source": verification.get("url", "") if item.get("showSource", True) else "",
            "sourceLabel": item.get("sourceLabel", verification.get("sourceLabel", "")),
            "sourceLinkLabel": verification_link_label(verification_link_type(verification)),
        })
    return resolved


def sync_featured_alumni(people: list[dict[str, Any]]) -> None:
    write_text(ASSETS_DIR / "data/featured-alumni.json", json.dumps(
        {"items": resolve_featured_alumni(people)}, ensure_ascii=False, indent=2
    ) + "\n")
...
def contact_mailto(route: str) -> str:
    fields = {
        "general": {
            "cc": "james_spencer@hms.harvard.edu",
            "subject": "[Bernhardt Lab website] General inquiry",
        },
        "training": {
            "subject": "[Bernhardt Lab website] Training inquiry",
            "body": "Name:\r\nCurrent institution and training stage:\r\nOpportunity sought:\r\nProposed start date and availability:\r\nWhy the Bernhardt Lab:\r\nRelevant experience:\r\nCV or profile link:\r\n",
        },
    }
    return "mailto:thomas_bernhardt@hms.harvard.edu?" + urlencode(fields[route], quote_via=quote)
...
def sync_homepage_content(text: str) -> str:
    ...
    for route in ("general", "training"):
        pattern = rf'(<a\b[^>]*data-contact-route="{route}"[^>]*href=")[^"]*(")'
        text, count = re.subn(pattern, lambda m: m[1] + escape(contact_mailto(route)) + m[2], text)
        if count != 1:
            raise RuntimeError(f"Expected one {route} contact link")
    return text
...
def render_alumni_cards(people: list[dict[str, Any]], root_prefix: str, flat: bool) -> str:
    ...
    for person in people:
        ...
        link_type = verification_link_type(verification)
        has_current_profile_link = bool(verified_url and link_type == "profile")
        source_prefix = verification_source_prefix(link_type)
        ...
        verified_html = (
            f'\n            <p class="alumni-source"><strong>{escape(source_prefix)}:</strong> <a class="alumni-inline-link" href="{escape(verified_url)}" target="_blank" rel="noreferrer">{escape(verified_source or "Verification source")}</a></p>'
            if verified_url
            else ""
        )
        verified_link_html = (
            f'<a class="alumni-link" href="{escape(verified_url)}" target="_blank" rel="noreferrer">{escape(verification_link_label(link_type))}</a>'
            if has_current_profile_link
            else ""
        )
        ...
        cards.append(
            ...
            <div class="alumni-top">{'<span class="alumni-verified">External profile</span>' if has_current_profile_link else ''}</div>
...
def render_profile_nav(root_prefix: str, flat: bool, current_section: str) -> str:
    ...
    return f'''          <button class="nav-toggle" type="button" aria-expanded="false" aria-controls="site-nav">Menu</button>
          <nav id="site-nav" class="site-nav" aria-label="Profile navigation">
...
def render_profile_hero_meta(role: str, lab_dates: str) -> str:
    rows = []
    if role:
        rows.append(f'<p class="role"><strong>Role:</strong> <span>{escape(role)}</span></p>')
    if lab_dates:
        rows.append(
            f'<p class="profile-lab-dates"><strong>Lab dates:</strong> <span>{escape(lab_dates)}</span></p>'
        )
    if not rows:
        return ""
    content = "\n                ".join(rows)
    return f'''<div class="profile-hero-meta">
                {content}
              </div>'''
...
def render_current_profile(person: dict[str, Any], flat: bool) -> str:
    ...
    education_section = (
        f'''\n          <div class="profile-grid profile-grid--current">{education_panel}
          </div>'''
        if education_panel
        else ""
    )
...
{favicon_links(root_prefix)}
    <link rel="stylesheet" href="{escape(root_prefix)}assets/profile.css?v={asset_version('profile.css')}" />
...
{render_profile_nav(root_prefix, flat, "team")}
      ...
      <main id="main-content" class="main">
        <section class="profile-shell">
          <article class="profile-hero profile-hero--current">
            <div class="profile-copy">
              <p class="eyebrow">Team Profile</p>
              <h1>{escape(name)}</h1>
              {render_profile_hero_meta(role, lab_dates)}
              <div class="actions">
                {' '.join(action_links)}
              </div>
            </div>
    ...
    <script type="module" src="{escape(root_prefix)}assets/site.js?v={asset_version('site.js')}"></script>
  </body>
</html>
'''
...
def render_alumni_profile(person: dict[str, Any], flat: bool) -> str:
    ...
    source_label = clean_text(verification.get("sourceLabel")) or "Bernhardt lab records"
    name = clean_text(person.get("name"))
    role = clean_text(person.get("labRole")) or "Former lab member"
    current_role = clean_text(person.get("currentRole"))
    lab_dates = clean_text(person.get("labDates"))
    ...
    verified_source_html = (
        f'\n              <p><strong>{escape(source_prefix)}:</strong> <a class="profile-link" href="{escape(verified_url)}" target="_blank" rel="noreferrer">{escape(verified_source or "Verification source")}</a></p>'
        if verified_url
        else ""
    )
      ...
      <main id="main-content" class="main">
        <section class="profile-shell">
          <article class="profile-hero">
            <div class="profile-copy">
              ...
              <div class="actions">
                ...
                {f'<a class="button button-secondary" href="{escape(verified_url)}" target="_blank" rel="noreferrer">View external profile</a>' if has_current_profile_link else ''}
              </div>
            </div>
            <figure class="profile-photo-wrap">
          ...
          <div class="profile-grid">
            {current_role_panel}{education_panel}
            <article class="profile-panel">
              <h2>Sources</h2>
...
def sync_legacy_redirects(
    ...
    for person in alumni:
        ...
        former_team_profile = ROOT / TEAM_ROUTE / slug / "index.html"
        if former_team_profile.exists():
            write_text(
                former_team_profile,
                render_redirect_page(
                    ...
                    "../../",
                    f"{clean_text(person.get('name'))} Redirect",
                ),
            )
...
def build_canonical_pages() -> None:
    ...
    index_text = sync_homepage_content(index_text)
...
def build_flat_pages() -> None:
    ...
    for person in alumni:
        slug = clean_text(person.get("slug"))
        write_text(FLAT_DIR / f"alumni-{slug}.html", generated_html(render_alumni_profile(person, flat=True)))
        former_team_profile = FLAT_DIR / f"team-{slug}.html"
        if former_team_profile.exists():
            write_text(
                former_team_profile,
                generated_html(
...
def validate_homepage_team_grid(path: Path, expected_cards: int) -> None:
    text = read_text(path)
    if text.count('id="home-team-preview"') != 1:
        raise RuntimeError(f"{path}: expected exactly one home-team-preview container")
    if any(f'id="{control}"' in text for control in ("people-sort", "people-search", "role-filters", "people-grid")):
...
def validate_site_quality_metadata() -> None:
    ...
    for path in public_paths:
        text = read_text(path)
        if '<script type="module"' in text:
            maps = re.findall(r'<script type="importmap" id="site-module-map">(.*?)</script>', text, re.S)
            base = FLAT_DIR if path.is_relative_to(FLAT_DIR) else ROOT
            prefix = "../" * (len(path.relative_to(base).parts) - 1)
            if len(maps) != 1 or json.loads(maps[0]).get("imports") != module_imports(prefix):
                version_errors.append(f"{path.relative_to(ROOT)} has a missing or stale module map")
            elif text.index('type="importmap"') > text.index('type="module"'):
                version_errors.append(f"{path.relative_to(ROOT)} loads its module map too late")
