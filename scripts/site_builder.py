...
STYLE_VERSION = "20260908a"
PROFILE_STYLE_VERSION = "20260908a"
ALUMNI_STYLE_VERSION = "20260908a"
MAIN_JS_VERSION = "20260908a"
ENVELOPE_CSS_VERSION = "20260710b"
ENVELOPE_CONFIG_VERSION = "20260318a"
ENVELOPE_JS_VERSION = "20260710a"
SITE_JS_VERSION = "20260908a"
...
def resolve_featured_alumni(people: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for item in load_featured_alumni_items():
        ...
        if clean_text(item.get("name")) != clean_text(person["name"]):
            raise RuntimeError(f"Featured name disagrees with people.json: {slug}")
        verification = person.get("verification") or {}
        if item.get("source") and item["source"] != verification.get("url"):
            raise RuntimeError(f"Featured source disagrees with people.json: {slug}")
        if item.get("roleFromPeople") and item.get("currentRole") != person.get("currentRole"):
            raise RuntimeError(f"Featured role disagrees with people.json: {slug}")
        # Keep approved editorial role labels; derive source presentation centrally.
        resolved.append({
            **item,
            "name": person["name"],
...
{render_profile_nav(root_prefix, flat, "team")}
    ...
    <script type="module" src="{escape(root_prefix)}assets/site.js?v={SITE_JS_VERSION}"></script>
  </body>
</html>
'''
